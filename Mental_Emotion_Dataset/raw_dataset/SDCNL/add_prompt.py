...
questions =  ["Does this person suffer from suicidal potential or depression?", "Determine whether this person is depressed or wants to suicide?"]
output_constraints = "Only choose one answer from the following options: Suicidal, Depression"

names = ["train", "test"]

for name in names:
	with open(f"./raw/{name}.csv", "r") as input_file:
		with open(f"../../prompted_dataset/SDCNL/{name}.csv", "w") as output_file:
